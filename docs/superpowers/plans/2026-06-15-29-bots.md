...
### Task 6: Wire card-counting (voids + trumps-out) into scoring

**Files:**
- Modify: `src/bots/heuristics.ts` (`scorePlay`)
- Test: `tests/bots/heuristics-counting.test.ts`

Delivers spec decision #1: the tracked `voids` and `trumpsOut` must influence play. Two concrete uses — (a) a "boss" lead is worth less when a known-void opponent can trump it; (b) trumping to win costs less as trumps deplete (fewer left to conserve).

- [ ] **Step 1: Write failing tests** — `tests/bots/heuristics-counting.test.ts`
...
- [ ] **Step 2: Run, verify fail** — `npx vitest run tests/bots/heuristics-counting.test.ts` → FAIL.

- [ ] **Step 3: Modify `scorePlay` in `src/bots/heuristics.ts`**

Add to the existing imports the `Seat` type and the `trumpsOut` helper:

```ts
import type { Card, Rank, Seat, Suit, TrumpMode } from '../engine/types';
```
(extend the existing `../engine/types` import to include `Seat`) and extend the cardMemory import:
```ts
import { isBoss, trumpsOut, type CardMemory } from './cardMemory';
```

In the **leading** branch of `scorePlay`, replace:
```ts
    if (isBoss(card, mem)) s += 10;
```
with:
...
In the **following / weWin** branch, replace:
```ts
    if (trump !== null && card.suit === trump) s -= 2; // mild trump cost
```
with:
...
- [ ] **Step 4: Run, verify pass** — `npx vitest run tests/bots/` → all pass (incl. new file). `npx tsc --noEmit` → exit 0. Run the simulator once: `npx vitest run tests/engine/simulate-bots.test.ts` → still PASS (invariants + >25% bid-met hold).

- [ ] **Step 5: Commit**

```bash
git add src/bots/heuristics.ts tests/bots/heuristics-counting.test.ts
git commit -m "feat(bots): use voids and trumps-out in card-play scoring"
```
